...
impl BleClient {
    ...
    async fn pairing_request(
        ...
        let response = async {
            loop {
                let b = replies
                    ...
                    .await
                    .map_err(|e| BleError::Native(e.to_string()))?;
                if b.starts_with(&[0xaa, 0xbb, frame[2]])
        ...
        tokio::select! {biased;_=s.cancel.cancelled()=>Err(BleError::Cancelled),r=timeout(Duration::from_secs(4),response)=>r.map_err(|_|BleError::Timeout("pairing control reply"))?}
    }
    pub async fn read_host_info(&self) -> Result<[host_info::HostInfo; 2]> {
        let generation = self.status().generation;
        let mut hosts = Vec::new();
        for slot in 0..2 {
            let mut parts: [Vec<u8>; 3] = Default::default();
            for part in 0..3 {
                parts[part as usize] = self
                    .pairing_request(&host_info::query(slot, part)?, None, generation)
                    .await?;
            }
            hosts.push(host_info::decode(slot, &parts)?);
        }
        Ok([hosts.remove(0), hosts.remove(0)])
    }
    pub async fn register_host(&self, name: &str, system: u8) -> Result<()> {
        let generation = self.status().generation;
        // Read-only capability probe before any registration on older firmware.
        let probe = self
            .pairing_request(&host_info::query(0, 0)?, None, generation)
            .await?;
        if probe.len() != 20 || probe[..4] != [0xaa, 0xbb, 0xa8, 0] {
            return Err(BleError::Invalid("固件不支持名称上报".into()));
        }
        let id = routing::next_management_request();
        let frames = host_info::registration(name, system, id)?;
        let mut slot = None;
        for (part, frame) in frames.iter().enumerate() {
            let b = self.pairing_request(frame, Some(id), generation).await?;
            let actual = host_info::confirm(&b, id, part as u8)?;
            if slot.is_some_and(|s| s != actual) {
                return Err(BleError::Invalid("上报期间槽位已互换，请重新上报".into()));
            }
            slot = Some(actual);
        }
        Ok(())
    }
    pub async fn read_pairing(&self) -> Result<routing::Details> {
        let b = self
            .pairing_request(&routing::DETAILS_QUERY, None, self.status().generation)
            .await?;
        routing::parse_details(&b)
    }
    pub async fn read_policy(&self) -> Result<reset::Policy> {
        let b = self
            .pairing_request(&reset::QUERY, None, self.status().generation)
            .await?;
        reset::parse(&b)
    }
    pub async fn manage_pairing(&self, action: routing::ManagementAction) -> Result<()> {
        let generation = self.status().generation;
        let capability = self
            .pairing_request(&routing::DETAILS_QUERY, None, generation)
            .await?;
        routing::parse_details(&capability)?;
        let id = routing::next_management_request();
        let frame = action.frame(id);
        let ack = self.pairing_request(&frame, Some(id), generation).await?;
        routing::management_confirmed(&ack, action, id)
    }
