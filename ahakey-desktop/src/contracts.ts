...
export interface Snapshot {
  ...
  usb: { supported: boolean; present?: boolean | null; status: HardwareStatus | null; error: string | null };
  caption: Caption;
...
export function deviceConnectionLabel(snapshot: (Pick<Snapshot, "device" | "nativeKeyTestEnabled"> & Partial<Pick<Snapshot, "usb">>) | null): string {
  if (!snapshot) return "连接本机服务…";
  const source=snapshot.device?.transport;
  if (!source || !snapshot.device.status) {
    if (snapshot.usb?.error) return snapshot.usb.present === true
      ? "USB 已识别 · 状态读取失败"
      : "本机服务运行 · USB 通信异常";
    return "本机服务运行 · 等待键盘";
  }
