...
// A transparent monochrome waveform lets macOS supply menu-bar contrast.
    ...
    *app.state::<Runtime>().tray_controls.lock().unwrap() = Some(Controls {
    ...
    });
    {
        ...
        let tray = tauri::tray::TrayIconBuilder::with_id("main-tray")
            ...
            .build(app)?;
        debug_assert_eq!(tray.id().as_ref(), "main-tray");
        app.state::<Runtime>()
            .tray
...
mod icon_tests {
    #[test]
    fn macos_bundle_overlay_matches_tauri_schema() {
        let mut base: serde_json::Value =
            serde_json::from_str(include_str!("../tauri.conf.json")).unwrap();
        let overlay: serde_json::Value =
            serde_json::from_str(include_str!("../tauri.macos.conf.json")).unwrap();
        for (key, value) in overlay["bundle"].as_object().unwrap() {
            base["bundle"][key] = value.clone();
        }
        let _: tauri::Config = serde_json::from_value(base).unwrap();
    }
    #[test]
    fn menu_bar_template_is_visible_monochrome_and_transparent() {
